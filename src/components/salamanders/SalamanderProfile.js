...
class SalamanderProfile extends Component {
    constructor(props) {
        super(props);
        this.state = {
            salamanderList: [
    ...
    componentDidMount() {
        const salamander = this.setState(this.state);
        axios.get(salamander).then(response => response.data)
            .then((data) => {
                this.setState({salamander: data});
            });
    }

    render() {
        return (
            this.state.salamanderList.filter(salamander => salamander.id.toString() === this.props.match.params.id).map((salamander, index) =>
            <div className="scrolling">
                <div className="scrolling-container">

                    <img className="scrolling-img" src={salamander.img[0]} alt="salamander"/>
                    <img className="scrolling-img" src={salamander.img[1]} alt="salamander"/>
                    <img className="scrolling-img" src={salamander.img[2]} alt="salamander"/>
                    <img className="scrolling-img" src={salamander.img[3]} alt="salamander"/>
                    <img className="scrolling-img" src={salamander.img[4]} alt="salamander"/>

                </div>
                <p className="about">{salamander.about}</p>
                <p className="facts">{salamander.facts}</p>
            </div>
        ))
    }
}

export default SalamanderProfile;